...
description: Scaffolds a complete new SipSync mini-game end-to-end — backend
  ...
  tutorial preview — from a rules list and design notes. Runs a spec-
  confirmation checkpoint before writing any code. Use when adding a brand
  new mini-game to SipSync, not for editing an existing one.
---
...
7. `frontend/components/tutorials/<Id>Tutorial.tsx` implementing the
   ...
   `frontend/constants/tutorials.ts` keyed `tutorial.<id>`.
...
rethinking, not that the constraint should be bent.
