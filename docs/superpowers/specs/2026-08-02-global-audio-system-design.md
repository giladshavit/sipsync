...
- **Autoplay-safe play helper**: wraps every `.play()` call (from `playBGM` and from unmuting) in try/catch. On web, a blocked autoplay throws or rejects with a `DOMException`; when caught, `isMuted` is forced to `true` in memory only — not persisted to `AsyncStorage` — instead of crashing or leaving playback in an inconsistent state, since a blocked autoplay is a transient per-page-load browser condition, not a user choice; persisting it would silently carry a forced mute into future sessions. On native this branch is not expected to trigger.
